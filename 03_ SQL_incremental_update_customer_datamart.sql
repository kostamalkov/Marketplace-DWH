...
-- DDL таблицы инкрементальных загрузок: load_dates_customer_report_datamart
...
WITH
-- Дельта изменений. запрос, который получает только обновлённые или новые данные из хранилища в DWH
dwh_delta AS (
	SELECT 	
...
-- делаем выборку клиентов ручной работы, по которым были изменения в DWH. По этим мастерам данные в витрине нужно будет обновить
dwh_update_delta AS (
	SELECT 	
...
-- делаем расчёт витрины по новым данным. Этой информации по клиентам в рамках расчётного периода раньше не было, это новые данные. 
dwh_delta_insert_result AS (
	SELECT 

			T4.customer_id AS customer_id, --идентификатор заказчика; 
			T4.customer_name AS customer_name, -- Ф. И. О. заказчика;
			T4.customer_address AS customer_address, --адрес заказчика;
			T4.customer_birthday AS customer_birthday, --дата рождения заказчика;
			T4.customer_email AS customer_email, --электронная почта заказчика;
			T4.customer_money AS customer_money, --сумма, которую потратил заказчик;
			T4.platform_money AS platform_money, --сумма, которую заработала платформа от покупок заказчика за месяц (10% от суммы, которую потратил заказчик);
			T4.count_order AS count_order, --количество заказов у заказчика за месяц;
			T4.avg_price_order AS avg_price_order, -- средняя стоимость одного заказа у заказчика за месяц;
			T4.median_time_order_completed AS median_time_order_completed, -- медианное время в днях от момента создания заказа до его завершения за месяц;
			T4.product_type AS top_product_category, --самая популярная категория товаров у этого заказчика за месяц;
            T4.craftsman_id AS top_craftsman_id, --идентификатор самого популярного мастера ручной работы у заказчика. Если заказчик сделал одинаковое количество заказов у нескольких мастеров, возьмите любого;
			T4.count_order_created AS count_order_created, --количество созданных заказов за месяц;
			T4.count_order_in_progress AS count_order_in_progress, --количество заказов в процессе изготовки за месяц;
			T4.count_order_delivery AS count_order_delivery, -- количество заказов в доставке за месяц;
			T4.count_order_done AS count_order_done, -- количество завершённых заказов за месяц;
			T4.count_order_not_done AS count_order_not_done, -- количество незавершённых заказов за месяц;
			T4.report_period AS report_period --отчётный период, год и месяц.
                                    ...
                                    SELECT
                                            ...
                                            FROM dwh_delta AS dd
                                                GROUP BY dd.customer_id, dd.craftsman_id
                                                    ORDER BY count_craftsman DESC) as T3_1 ON T2.customer_id = T3_1.customer_id_for_top_craftsman
